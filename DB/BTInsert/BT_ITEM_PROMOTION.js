...
(function (exports) {
	...
	inst.BT_ITEM_PROMOTION = function (sequelize, path, callback) {
		base_table = sequelize.define('BT_ITEM_PROMOTION', {
			PROMOTION_ID: { type: sequelize_module.INTEGER, allowNull: false },
			STEP: { type: sequelize_module.INTEGER, allowNull: false },
			NEED_GOLD: { type: sequelize_module.INTEGER, allowNull: false },
			LIMIT_ITEM_LEVEL: { type: sequelize_module.INTEGER, allowNull: false },
				...
				for (var i in read.Rows) {
					(function (i) {
						base_table.findOrCreate({
							PROMOTION_ID: read.Rows[i].PromotionID,
						...
						},
						{
							...
							LIMIT_ITEM_LEVEL: read.Rows[i].LimitItemLevel,
						...
						.success(function (anotherTask) {
							...
							if (read.Rows.length - 1 == i && typeof callback == 'function') callback('BT_ITEM_PROMOTION');
						})
						.error(function (err) {
							console.log(err);
						});
